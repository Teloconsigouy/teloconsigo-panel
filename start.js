...
const server = http.createServer((req, res) => {
  ...
  // Mientras estabilizamos Railway / MeLi Ads / IA, todo entra como admin.
  // El login seguro se vuelve a implementar después.
  ...
  // LOGIN TEMPORAL DESACTIVADO:
  // Si alguien entra a login.html, lo mandamos directo al panel.
  ...
  if (pathName === '/api/meli') {
    (async () => {
      ...
      try {
        ...
        audit(session, 'meli_ads_api', { cuenta, method: req.method, status: r.status, action, params: paramsObj });

        // Si n8n falla o viene vacío justo al entrar/salir, no dejamos el panel en blanco.
        if (hasError) {
          const cache = cachedMeliResponse(cuenta);
          jsonResp(res, 200, { ...cache, warning: data?.error?.message || data?.message || `n8n respondió HTTP ${r.status}` });
          return;
        }
        ...
        if ((cache.campaigns || []).length) {
          jsonResp(res, 200, { ...cache, warning: e.message });
        } else {
    ...
    (async () => {
      try {
        ...
        const r = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({
            model:      data.model || process.env.ANTHROPIC_MODEL || 'claude-3-5-sonnet-20241022',
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('  TELOCONSIGO + TOP SHOP — Panel v4 (login temporal desactivado)');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
