...
// ═══════════════════════════════════════════════
// Datos persistentes:
// - En local se mantiene ./data como hasta ahora.
// - En Railway, si existe un Volume montado, se usa /data (o RAILWAY_VOLUME_MOUNT_PATH).
// - TLC_DATA_DIR permite forzar otra ruta sin tocar el codigo.
const DEFAULT_DATA_DIR = path.join(__dirname, 'data');
const IS_RAILWAY = !!(process.env.RAILWAY_ENVIRONMENT || process.env.RAILWAY_ENVIRONMENT_NAME || process.env.RAILWAY_PROJECT_ID || process.env.RAILWAY_SERVICE_ID);
const DATA_DIR = process.env.TLC_DATA_DIR
  || process.env.RAILWAY_VOLUME_MOUNT_PATH
  || (IS_RAILWAY ? '/data' : DEFAULT_DATA_DIR);
...
// Primera ejecucion con Volume: copia la memoria previa del repo solo si el destino esta vacio.
// Nunca pisa archivos que ya existen en el Volume.
function migrateLegacyDataToPersistentDir() {
  ensureDataDir();
  if (path.resolve(DATA_DIR) === path.resolve(DEFAULT_DATA_DIR)) return;
  const names = [
    'inbox-state.json',
    'audit-log.json',
    'publications-cache.json',
    'publicador-borradores.json',
    'precios.json',
    'meli-oauth-tokens.json',
  ];
  for (const name of names) {
    const from = path.join(DEFAULT_DATA_DIR, name);
    const to = path.join(DATA_DIR, name);
    try {
      if (!fs.existsSync(to) && fs.existsSync(from)) fs.copyFileSync(from, to);
    } catch (e) {
      console.warn(`[persistencia] No se pudo migrar ${name}:`, e.message);
    }
  }
  // Compatibilidad con instalaciones viejas que tenian precios.json en la raiz.
  try {
    const to = path.join(DATA_DIR, 'precios.json');
    if (!fs.existsSync(to) && fs.existsSync(ROOT_PRECIOS_FILE)) fs.copyFileSync(ROOT_PRECIOS_FILE, to);
  } catch (e) {
    console.warn('[persistencia] No se pudo migrar precios.json de raiz:', e.message);
  }
}

migrateLegacyDataToPersistentDir();
console.log(`[persistencia] DATA_DIR activo: ${DATA_DIR}`);
