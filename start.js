...
function parsePublicadorHtml(html, pageUrl) {
  ...
  // Fotos: si la pagina declara un Product en JSON-LD, usamos SOLO las imagenes
  // asociadas a ese producto. Esto evita logos, banners, iconos, productos relacionados
  // y otras imagenes globales de la pagina. Solo si el proveedor no expone imagenes
  // de producto en datos estructurados hacemos fallback al extractor general.
  const productImages = [];
  const addProductImage = (v) => {
    if (!v) return;
    if (Array.isArray(v)) { v.forEach(addProductImage); return; }
    if (typeof v === 'object') { addProductImage(v.url || v.contentUrl || v.src); return; }
    const u = normalizePublicadorImageUrl(String(v), pageUrl);
    if (isUsefulPublicadorImage(u) && !productImages.includes(u)) productImages.push(u);
  };
  addProductImage(product.image);
  // Fallback conservador: una imagen social (og/twitter) suele ser la foto principal del
  // producto. Si tampoco existe, tomamos solo la primera imagen util de la pagina. Es
  // preferible informar 1 foto real antes que contaminar el borrador con banners/logos.
  const socialProductImage = getMetaContent(htmlStr, ['og:image', 'twitter:image']);
  const fallbackImages = extractPublicadorImages(htmlStr, pageUrl);
  const pageImages = productImages.length
    ? productImages.slice(0, 12)
    : (socialProductImage ? [normalizePublicadorImageUrl(socialProductImage, pageUrl)].filter(isUsefulPublicadorImage) : fallbackImages.slice(0, 1));
  const imageExtractionSource = productImages.length ? 'product_jsonld' : (pageImages.length && socialProductImage ? 'product_social_meta' : 'page_first_image_fallback');
  return {
    scrapedTitle,
    scrapedDescription,
    images: pageImages,
...
function extractSupplierPriceCurrency(html) {
  const raw = String(html || '');
  const text = cleanPublicadorText(raw).replace(/\u00a0/g, ' ');
  const candidates = [];
  const add = (price, currency, source) => {
    let n = Number(price);
    if (!Number.isFinite(n) || n <= 0) return;
    const c = String(currency || '').toUpperCase();
    if (!['UYU','USD'].includes(c)) return;
    candidates.push({ price:n, currency:c, source });
  };
  const parseNum = (v) => {
    let x = String(v || '').trim().replace(/\s/g,'');
    if (!x) return NaN;
    if (/^\d{1,3}(?:\.\d{3})+,\d{1,2}$/.test(x)) x=x.replace(/\./g,'').replace(',','.');
    else if (/^\d{1,3}(?:,\d{3})+\.\d{1,2}$/.test(x)) x=x.replace(/,/g,'');
    else if (/^\d{1,3}(?:\.\d{3})+$/.test(x)) x=x.replace(/\./g,'');
    else if (/^\d{1,3}(?:,\d{3})+$/.test(x)) x=x.replace(/,/g,'');
    else if (x.includes(',') && !x.includes('.')) x=x.replace(',','.');
    return Number(x);
  };
  // JSON-LD / microdata / meta commerce.
  for (const m of raw.matchAll(/"priceCurrency"\s*:\s*"(UYU|USD)"[\s\S]{0,300}?"price"\s*:\s*"?([0-9][0-9.,]*)"?/gi)) add(parseNum(m[2]),m[1],'jsonld');
  for (const m of raw.matchAll(/"price"\s*:\s*"?([0-9][0-9.,]*)"?[\s\S]{0,300}?"priceCurrency"\s*:\s*"(UYU|USD)"/gi)) add(parseNum(m[1]),m[2],'jsonld');
  for (const m of raw.matchAll(/(?:itemprop=["']price["'][^>]*(?:content|value)=["']([0-9][0-9.,]*)["']|property=["']product:price:amount["'][^>]*content=["']([0-9][0-9.,]*)["'])/gi)) {
    const n=parseNum(m[1]||m[2]);
    const near=raw.slice(Math.max(0,m.index-500),Math.min(raw.length,m.index+700));
    const cm=near.match(/(?:priceCurrency["'][^>]*(?:content|value)=["']|product:price:currency["'][^>]*content=["'])(UYU|USD)/i);
    if(cm)add(n,cm[1],'meta');
  }
  // Visible supplier price. U$S/USD => USD; $U/UYU/pesos => UYU.
  const visiblePatterns = [
    { re:/(?:U\$S|US\$|USD)\s*[:$]?\s*([0-9][0-9.,]*)/gi, cur:'USD' },
    { re:/([0-9][0-9.,]*)\s*(?:U\$S|US\$|USD)\b/gi, cur:'USD' },
    { re:/(?:\$U|UYU)\s*[:$]?\s*([0-9][0-9.,]*)/gi, cur:'UYU' },
    { re:/([0-9][0-9.,]*)\s*(?:\$U|UYU)\b/gi, cur:'UYU' },
    { re:/(?:precio|price)\s*:?\s*\$\s*([0-9][0-9.,]*)/gi, cur:'UYU' }
  ];
  for(const p of visiblePatterns) for(const m of text.matchAll(p.re)) add(parseNum(m[1]),p.cur,'visible');
  // Prefer structured data, then a visible value. Ignore implausibly tiny/huge accidental matches.
  const valid=candidates.filter(x=>x.price>=1 && x.price<10000000);
  valid.sort((a,b)=>({jsonld:0,meta:1,visible:2}[a.source]??9)-({jsonld:0,meta:1,visible:2}[b.source]??9));
  return valid[0] || null;
}

async function fetchSupplierPage(url, timeoutMs=15000) {
  const controller=new AbortController();
  const timer=setTimeout(()=>controller.abort(),timeoutMs);
  try {
    const r=await fetch(url,{redirect:'follow',signal:controller.signal,headers:{'User-Agent':'Mozilla/5.0 TLC-Publicador-V5.0.6/1.0','Accept':'text/html,application/xhtml+xml'}});
    if(!r.ok)throw new Error(`No se pudo leer el link del proveedor (${r.status}).`);
    const buf=Buffer.from(await r.arrayBuffer());
    const contentType=r.headers.get('content-type')||'';
    const charset=(contentType.match(/charset=([^;]+)/i)?.[1]||'').toLowerCase();
    let html=buf.toString('utf8');
    if(charset&&!charset.includes('utf')&&(charset.includes('iso')||charset.includes('latin')||charset.includes('windows')))html=new TextDecoder('latin1').decode(buf);
    return html;
  } catch(e) {
    if(e?.name==='AbortError')throw new Error('La pagina del proveedor demoro demasiado en responder.');
    throw e;
...
function publicadorMissingRequiredAttributes(requiredAttrs, payloadAttributes) {
  const required = Array.isArray(requiredAttrs) ? requiredAttrs : [];
  const attrs = Array.isArray(payloadAttributes) ? payloadAttributes : [];
  const has = (id) => attrs.some(x => String(x && x.id || '').toUpperCase() === String(id || '').toUpperCase() && String(x.value_name || x.value_id || '').trim());
  const hasGtin = has('GTIN');
  const hasNoGtinReason = has('EMPTY_GTIN_REASON');
  return required.filter(a => {
    const id = String(a && a.id || '').toUpperCase();
    // Mercado Libre trata GTIN y EMPTY_GTIN_REASON como alternativas: si el producto
    // no tiene codigo universal y existe un motivo de ausencia valido, GTIN deja de faltar.
    if (id === 'GTIN' || id === 'EMPTY_GTIN_REASON') return !(hasGtin || hasNoGtinReason);
    return !has(id);
  }).map(a => ({
    id:a.id,
    name:a.name||a.id,
    value_type:a.value_type||'string',
    allowed_values:Array.isArray(a.values)?a.values.slice(0,20):[]
  }));
}
...
const MCP_SERVER_VERSION = '5.0.5';
...
const MCP_TOOLS = [
  ...
  { name:'meli_catalog_competition', title:'Competencia de Catalogo', description:'Revisa publicaciones de Catalogo y consulta price_to_win v2 para identificar cuales estan ganando, compartiendo primer lugar, perdiendo o listadas sin competir. Antes de filtrar Catalogo hidrata cada publicacion candidata con GET /items/{MLU}, porque el cache resumido puede no incluir catalog_listing/catalog_product_id. Puede filtrar por texto, por ejemplo INGCO. Solo lectura.', inputSchema:{type:'object',properties:{cuenta:{type:'string',enum:['tlc','topshop','both'],default:'both'},q:{type:'string',default:'',description:'Filtro opcional por titulo, MLU o SKU antes de consultar el estado real. Ejemplo: INGCO.'},only_losing:{type:'boolean',default:true,description:'Si true devuelve detalle solo de competing y listed; el resumen siempre incluye todos los estados.'},refresh:{type:'boolean',default:true,description:'Actualiza la lista de publicaciones antes del analisis.'},max_items:{type:'integer',minimum:1,maximum:5000,default:5000}},additionalProperties:false}, annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:true} },
  { name:'meli_prepare_publication', title:'Preparar publicacion desde link', description:'Prepara una publicacion desde un link de proveedor siguiendo el flujo de Mercado Libre: extrae producto/precio/moneda y solo las fotos asociadas al producto, consulta categorias candidatas en MLU, selecciona la mejor candidata, consulta atributos/requisitos y muestra exactamente que falta. Si no hay GTIN, usa EMPTY_GTIN_REASON valido como alternativa cuando Mercado Libre lo admite. Precio y moneda son opcionales; stock SIEMPRE 50. Busca posibles duplicados y guarda borrador. NO publica.', inputSchema:{type:'object',properties:{url:{type:'string',minLength:8},currency:{type:'string',enum:['UYU','USD']},price:{type:'number',exclusiveMinimum:0},accounts:{type:'array',items:{type:'string',enum:['tlc','topshop']},minItems:1,maxItems:2}},required:['url'],additionalProperties:false}, annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:false,openWorldHint:true} },
...
async function mcpCallTool(name, args={}) {
  ...
  if (name === 'meli_catalog_competition') {
    ...
    const cache=loadPublicationsCache();
    const report={ok:true,read_only:true,source:'GET /items/{MLU} + GET /items/{MLU}/price_to_win?siteId=MLU&version=v2',query:q||null,accounts:{},summary:{candidates:0,hydrated:0,catalog_found:0,catalog_checked:0,winning:0,sharing_first_place:0,competing:0,listed:0,other:0,errors:0},items:[],truncated:false};
    const matchesQuery=(x)=>{if(!q)return true;const hay=[x?.id,x?.mlu,x?.title,x?.sku,x?.seller_custom_field].filter(Boolean).join(' ').toLowerCase();return hay.includes(q);};
    for(const c of accounts){
      try{
        if(refresh || !Array.isArray(cache[c]) || !cache[c].length){cache[c]=await fetchAllPublicationsDirect(c,{});cache.updatedAt=new Date().toISOString();savePublicationsCache(cache);}
        const base=(cache[c]||[]).filter(x=>x && String(x.status||'').toLowerCase()!=='closed' && matchesQuery(x));
        const selectedBase=base.slice(0,maxItems); if(base.length>selectedBase.length)report.truncated=true;
        const acc={candidates:base.length,hydrated:0,catalog_found:0,checked:0,winning:0,sharing_first_place:0,competing:0,listed:0,other:0,errors:0};
        report.summary.candidates+=base.length;
        const hydrated=[]; const concurrency=8;
        for(let i=0;i<selectedBase.length;i+=concurrency){
          const chunk=selectedBase.slice(i,i+concurrency);
          const rows=await Promise.all(chunk.map(async pub=>{try{const item=await meliApi(c, `/items/${encodeURIComponent(pub.id||pub.mlu)}`);return {...pub,...item,id:item?.id||pub.id||pub.mlu,mlu:item?.id||pub.mlu||pub.id,sku:pub.sku||item?.seller_custom_field||null,catalog_listing:Boolean(item?.catalog_listing),catalog_product_id:item?.catalog_product_id||null};}catch(e){acc.errors++;report.summary.errors++;return null;}}));
          for(const row of rows)if(row){hydrated.push(row);acc.hydrated++;report.summary.hydrated++;}
        }
        const candidates=hydrated.filter(x=>x.catalog_listing || x.catalog_product_id);
        acc.catalog_found=candidates.length; report.summary.catalog_found+=candidates.length;
        for(let i=0;i<candidates.length;i+=concurrency){
          ...
          const rows=await Promise.all(chunk.map(async pub=>{
            try{
              ...
              const current=Number(d?.current_price ?? pub.price ?? 0)||0; const ptw=d?.price_to_win==null?null:Number(d.price_to_win);
              const diff=ptw==null?null:Number((current-ptw).toFixed(2)); const diffPct=(ptw!=null&&current>0)?Number((((current-ptw)/current)*100).toFixed(2)):null;
              return {cuenta:c,item_id:String(d?.item_id||pub.id||pub.mlu||''),title:pub.title||null,sku:pub.sku||pub.seller_custom_field||null,catalog_listing:Boolean(pub.catalog_listing),catalog_product_id:d?.catalog_product_id||pub.catalog_product_id||null,status:st,visit_share:d?.visit_share??null,current_price:current,currency_id:d?.currency_id||pub.currency_id||'UYU',price_to_win:ptw,price_reduction_needed:diff,price_reduction_percent:diffPct,competitors_sharing_first_place:d?.competitors_sharing_first_place??null,reason:Array.isArray(d?.reason)?d.reason:[],boosts:Array.isArray(d?.boosts)?d.boosts:[],opportunities:Array.isArray(d?.boosts)?d.boosts.filter(b=>b?.status==='opportunity'):[],winner:d?.winner?{item_id:d.winner.item_id||null,price:d.winner.price??null,currency_id:d.winner.currency_id||null,boosts:Array.isArray(d.winner.boosts)?d.winner.boosts:[]}:null};
            }catch(e){acc.errors++;report.summary.errors++;return {cuenta:c,item_id:String(pub.id||pub.mlu||''),title:pub.title||null,sku:pub.sku||null,catalog_listing:Boolean(pub.catalog_listing),catalog_product_id:pub.catalog_product_id||null,error:String(e?.message||e)};}
          }));
          for(const row of rows){if(row.error || !onlyLosing || row.status==='competing' || row.status==='listed')report.items.push(row);}
        }
        acc.checked=candidates.length; report.summary.catalog_checked+=candidates.length; report.accounts[c]=acc;
    ...
    report.updatedAt=cache.updatedAt||new Date().toISOString(); return report;
  }

  if (name === 'meli_prepare_publication') {
    const url=String(args.url||'').trim();
    const accounts=(Array.isArray(args.accounts)&&args.accounts.length?args.accounts:['tlc','topshop']).map(normalizeCuentaKey).filter((v,i,a)=>['tlc','topshop'].includes(v)&&a.indexOf(v)===i);
    if(!/^https?:\/\//i.test(url))throw new Error('Falta un link valido del producto.');
    if(!accounts.length)throw new Error('Selecciona al menos una cuenta.');

    const html=await fetchSupplierPage(url,15000);
    const detected=extractSupplierPriceCurrency(html);
    const explicitPrice=args.price===undefined||args.price===null||args.price===''?null:Number(args.price);
    const explicitCurrency=args.currency===undefined||args.currency===null||args.currency===''?null:String(args.currency).toUpperCase();
    const price=explicitPrice!=null?explicitPrice:Number(detected?.price);
    const currency=explicitCurrency||String(detected?.currency||'').toUpperCase();
    if(!(price>0) || !['UYU','USD'].includes(currency)) {
      return {ok:false,read_only:true,needs_input:true,stock_rule:50,missing:[...(price>0?[]:['price']),...(['UYU','USD'].includes(currency)?[]:['currency'])],detected:detected||null,message:'No se pudo verificar automaticamente el precio y/o la moneda en la pagina del proveedor. Indicalos explicitamente; no se genero ningun borrador.'};
    }

    let scraped=parsePublicadorHtml(html,url);
    const meliIds=extractMeliIdsFromUrlOrHtml(url,html);
    if(meliIds.itemId||meliIds.catalogId||meliIds.userProductId){const imgs=await fetchMeliItemImagesFromPublicApi(meliIds.itemId,meliIds.catalogId,accounts[0]||'tlc',meliIds.userProductId);if(imgs.length)scraped={...scraped,images:mergePublicadorImages(scraped.images||[],imgs,url)};}
    const ai=await generatePublicadorContent({...scraped,url});
    // V5.0.7: primero preguntamos a Mercado Libre por categorias candidatas.
    // No se intenta construir la publicacion completa antes de conocer la categoria.
    const categoryQuery=String(ai.titulo_meli||scraped.scrapedTitle||'').trim();
    const categoryCandidates=await detectPublicadorCategoryCandidates(categoryQuery,8);
    const category=categoryCandidates[0] || await detectPublicadorCategory(categoryQuery);
    if(!category?.categoryId || category.categoryId==='MLU1574'){
      return {ok:false,read_only:true,needs_input:true,stock_rule:50,stage:'category',message:'Mercado Libre no devolvio una categoria publicable confiable para este producto. No se genero un borrador publicable.',category_query:categoryQuery,category_candidates:categoryCandidates};
    }
    // Recién con la categoria elegida consultamos su ficha de atributos/requisitos.
    ...
    const draft={id:crypto.randomUUID?crypto.randomUUID():crypto.randomBytes(16).toString('hex'),status:'preview',source:'mcp_v5_0_8',url,price,currency,priceSource:explicitPrice!=null?'user':detected?.source||'supplier_page',currencySource:explicitCurrency?'user':detected?.source||'supplier_page',stock:50,accounts,createdAt:new Date().toISOString(),...scraped,...ai,...category,categoryQuery,categoryCandidates,requiredAttributes,allAttributes};
    draft.stock=50; draft.price=price; draft.currency=currency; draft.meliPayload=buildPublicadorPayload(draft); draft.meliPayload.available_quantity=50;

    // Duplicate check: use the existing cache when available and only refresh an account if the cache is empty.
    // This avoids scanning thousands of publications on every draft preparation.
    const duplicateCandidates={};
    const terms=[draft.model,draft.brand,draft.titulo_meli].filter(Boolean).join(' ').toLowerCase();
    const tokens=terms.split(/\s+/).map(x=>x.replace(/[^a-z0-9]/gi,'')).filter(t=>t.length>=4);
    const cache=loadPublicationsCache();
    for(const c of ['tlc','topshop']){
      try{
        let pubs=Array.isArray(cache[c])?cache[c]:[];
        if(!pubs.length){pubs=await fetchAllPublicationsDirect(c,{});cache[c]=pubs;cache.updatedAt=new Date().toISOString();savePublicationsCache(cache);}
        duplicateCandidates[c]=(pubs||[]).filter(x=>{const hay=JSON.stringify([x.id,x.title,x.sku,x.seller_custom_field]).toLowerCase();return tokens.length&&tokens.filter(t=>hay.includes(t)).length>=Math.min(2,tokens.length);}).slice(0,10);
      }catch(e){duplicateCandidates[c]=[{error:e.message}];}
    }
    const data=loadPublicadorDrafts(); data.drafts=data.drafts||[]; data.drafts.unshift(draft); savePublicadorDrafts(data);
    const missing=publicadorMissingRequiredAttributes(requiredAttributes,draft.meliPayload?.attributes);
    draft.requiredMissing=missing;
    const di=(data.drafts||[]).findIndex(d=>String(d.id)===String(draft.id)); if(di>=0)data.drafts[di]=draft; savePublicadorDrafts(data);
    return {ok:true,read_only:true,draft_id:draft.id,stock_rule:50,workflow_stage:'category_requirements_checked',price_detection:{price,currency,price_source:draft.priceSource,currency_source:draft.currencySource},category_selection:{query:categoryQuery,selected:{id:draft.categoryId,name:draft.categoryName,domain_id:draft.domainId||null},candidates:categoryCandidates.slice(0,8)},preview:{title:draft.titulo_meli,description:draft.descripcion_meli,price,currency,stock:50,accounts,category:{id:draft.categoryId,name:draft.categoryName},brand:draft.brand,model:draft.model,image_extraction:{source:draft.imageExtractionSource||'page_fallback',count:(draft.images||[]).length},images:(draft.images||[]).slice(0,8),required_attributes:requiredAttributes.map(a=>({id:a.id,name:a.name||a.id,value_type:a.value_type||'string',values:Array.isArray(a.values)?a.values.slice(0,20):[]})),required_missing:missing,possible_duplicates:duplicateCandidates},ready_to_publish:missing.length===0,image_review_guidance:'Revisar las fotos antes de publicar. Si son insuficientes, de baja calidad o conviene crear/mejorar imagenes, hacerlo antes de llamar a meli_publish_draft.'};
  }
  if (name === 'meli_publish_draft') {
    ...
    if(Array.isArray(draft.requiredMissing)&&draft.requiredMissing.length){throw new Error('BORRADOR_INCOMPLETO: Mercado Libre exige datos que aun faltan para la categoria seleccionada: '+draft.requiredMissing.map(x=>x.name||x.id).join(', ')+'. Completar esos datos antes de publicar; no se inventaron valores.');}
