...
//  v4-online — login temporal abierto + /api/chat
// ═══════════════════════════════════════════════
...
// LOGIN TEMPORAL ABIERTO PARA RAILWAY
// IMPORTANTE: esto deja entrar con usuario admin y cualquier contraseña.
// Se usa para mantener la plataforma funcional; el login seguro se reimplementa después.
  ...
  if (req.method === 'POST' && pathName === '/api/chat') {
    (async () => {
      try {
        ...
        if (!apiKey) {
          jsonResp(res, 500, { error: { message: 'Falta configurar ANTHROPIC_API_KEY en Railway.' } });
          return;
        }
        ...
        catch { out = { error: { message: 'Anthropic devolvio una respuesta no-JSON', raw: text.slice(0, 400) } }; }
        if (!r.ok && !out.error) out.error = { message: `Anthropic respondio HTTP ${r.status}` };
