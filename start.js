...
const MCP_TOOLS = [
  ...
  { name:'meli_prepare_publication', title:'Preparar publicacion desde link', description:'Prepara una publicacion desde un link de proveedor siguiendo el flujo de Mercado Libre: extrae producto/precio/moneda, consulta categorias candidatas en MLU, selecciona la mejor candidata, consulta los atributos/requisitos de esa categoria y muestra exactamente que falta antes de publicar. Precio y moneda son opcionales; stock SIEMPRE 50. Busca posibles duplicados y guarda borrador. NO publica.', inputSchema:{type:'object',properties:{url:{type:'string',minLength:8},currency:{type:'string',enum:['UYU','USD']},price:{type:'number',exclusiveMinimum:0},accounts:{type:'array',items:{type:'string',enum:['tlc','topshop']},minItems:1,maxItems:2}},required:['url'],additionalProperties:false}, annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:false,openWorldHint:true} },
...
async function mcpCallTool(name, args={}) {
  ...
  if (name === 'meli_prepare_publication') {
    ...
    const draft={id:crypto.randomUUID?crypto.randomUUID():crypto.randomBytes(16).toString('hex'),status:'preview',source:'mcp_v5_0_7',url,price,currency,priceSource:explicitPrice!=null?'user':detected?.source||'supplier_page',currencySource:explicitCurrency?'user':detected?.source||'supplier_page',stock:50,accounts,createdAt:new Date().toISOString(),...scraped,...ai,...category,categoryQuery,categoryCandidates,requiredAttributes,allAttributes};
    ...
    const missing=requiredAttributes.filter(a=>!draft.meliPayload?.attributes?.some(x=>String(x.id)===String(a.id)&&String(x.value_name||x.value_id||'').trim())).map(a=>({id:a.id,name:a.name||a.id,value_type:a.value_type||'string',allowed_values:Array.isArray(a.values)?a.values.slice(0,20):[]}));
    ...
    return {ok:true,read_only:true,draft_id:draft.id,stock_rule:50,workflow_stage:'category_requirements_checked',price_detection:{price,currency,price_source:draft.priceSource,currency_source:draft.currencySource},category_selection:{query:categoryQuery,selected:{id:draft.categoryId,name:draft.categoryName,domain_id:draft.domainId||null},candidates:categoryCandidates.slice(0,8)},preview:{title:draft.titulo_meli,description:draft.descripcion_meli,price,currency,stock:50,accounts,category:{id:draft.categoryId,name:draft.categoryName},brand:draft.brand,model:draft.model,images:(draft.images||[]).slice(0,8),required_attributes:requiredAttributes.map(a=>({id:a.id,name:a.name||a.id,value_type:a.value_type||'string',values:Array.isArray(a.values)?a.values.slice(0,20):[]})),required_missing:missing,possible_duplicates:duplicateCandidates},ready_to_publish:missing.length===0,image_review_guidance:'Revisar las fotos antes de publicar. Si son insuficientes, de baja calidad o conviene crear/mejorar imagenes, hacerlo antes de llamar a meli_publish_draft.'};
  }
