  ...
  if (req.method === 'POST' && pathName === '/api/precios') {
    (async () => {
      try {
        ...
        const body = await readBody(req);
        if (['createProduct', 'updateProduct', 'deleteProduct'].includes(body.action || '')) {
          audit(session, 'prices_api', {
            module: 'prices',
            action: body.action || '',
            path: pathName,
            rowNumber: body.payload?.rowNumber || body.payload?.originalRowNumber || '',
            codigoTLC: body.payload?.codigoTLC || body.payload?.originalCodigoTLC || '',
            articulo: body.payload?.articulo || '',
          });
        }
