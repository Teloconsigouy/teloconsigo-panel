...
const MCP_TOOLS = [
  ...
  { name:'meli_prepare_publication', title:'Preparar publicacion desde link', description:'Prepara una publicacion de Mercado Libre desde un link de proveedor. Precio y moneda son opcionales: si no se indican, intenta extraerlos de la pagina del proveedor. Si se indican, tienen prioridad. El stock se fija SIEMPRE en 50. Extrae datos/fotos, detecta categoria y atributos, busca posibles duplicados en TLC/TOP SHOP y guarda un borrador. NO publica.', inputSchema:{type:'object',properties:{url:{type:'string',minLength:8},currency:{type:'string',enum:['UYU','USD']},price:{type:'number',exclusiveMinimum:0},accounts:{type:'array',items:{type:'string',enum:['tlc','topshop']},minItems:1,maxItems:2}},required:['url'],additionalProperties:false}, annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:false,openWorldHint:true} },
...
async function mcpCallTool(name, args={}) {
  ...
  if (name === 'meli_prepare_publication') {
    ...
    const draft={id:crypto.randomUUID?crypto.randomUUID():crypto.randomBytes(16).toString('hex'),status:'preview',source:'mcp_v5_0_6',url,price,currency,priceSource:explicitPrice!=null?'user':detected?.source||'supplier_page',currencySource:explicitCurrency?'user':detected?.source||'supplier_page',stock:50,accounts,createdAt:new Date().toISOString(),...scraped,...ai,...category,requiredAttributes,allAttributes};
