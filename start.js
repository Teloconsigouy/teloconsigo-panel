...
const MCP_TOOLS = [
  ...
  { name:'meli_question_context', title:'Contexto de una pregunta', description:'Obtiene la pregunta, la publicacion, su descripcion y preguntas/respuestas anteriores de esa publicacion para preparar una respuesta con contexto.', inputSchema:{type:'object',properties:{cuenta:{type:'string',enum:['tlc','topshop']},question_id:{type:['string','number']},history_limit:{type:'integer',minimum:1,maximum:100,default:50}},required:['cuenta','question_id'],additionalProperties:false}, annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:true} },
  ...
  { name:'meli_answer_question', title:'Responder pregunta', description:'ENVIA una respuesta a una pregunta de Mercado Libre. Solo ejecuta si confirm=true. Antes de usarla, mostrar al usuario la respuesta exacta y obtener su confirmacion explicita.', inputSchema:{type:'object',properties:{cuenta:{type:'string',enum:['tlc','topshop']},question_id:{type:['string','number']},text:{type:'string',minLength:1},confirm:{type:'boolean',description:'Debe ser true despues de confirmacion explicita del usuario.'}},required:['cuenta','question_id','text','confirm'],additionalProperties:false}, annotations:{readOnlyHint:false,destructiveHint:false,idempotentHint:false,openWorldHint:true} },
...
async function mcpCallTool(name, args={}) {
  ...
  if (name === 'meli_question_context') {
    ...
    return {ok:true,cuenta,question,item,description,history};
  }
  ...
  if (name === 'meli_answer_question') {
    ...
    const questionId=Number(args.question_id); const text=String(args.text||'').trim();
    ...
    return {ok:true,cuenta,action:'answer_question',question_id:questionId,text,response};
  }
...
async function mcpProcessMessage(req, body) {
  ...
  if (method === 'initialize') {
    return {protocol,status:200,payload:{jsonrpc:'2.0',id,result:{
      ...
      instructions:'Servidor MCP de TELOCONSIGO y TOP SHOP. Puede consultar cuentas, preguntas, contexto, ventas, reclamos y publicaciones. Las herramientas que modifican Mercado Libre exigen confirm=true y deben usarse solo despues de mostrar el cambio exacto y recibir confirmacion explicita del usuario.'
    }}};
  }
  ...
  if (method === 'tools/call') {
    ...
    try { const data=await mcpCallTool(name,args); return {protocol,status:200,payload:{jsonrpc:'2.0',id,result:mcpTextResult(data)}}; }
