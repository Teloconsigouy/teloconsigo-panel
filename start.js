...
async function mcpTextResult(data) {
  ...
  for (const url of imageUrls.slice(0, 8)) {
    try {
      const r = await fetch(url, {headers:{'User-Agent':'TELOCONSIGO-MCP/3.1'}});
      if (!r.ok) continue;
...
async function mcpCallTool(name, args={}) {
  ...
  if (name === 'meli_question_context') {
    ...
    const pictureUrls = Array.isArray(item?.pictures)
      ...
      : [];
    return {ok:true,cuenta,brand,question,item,description,history,picture_urls:pictureUrls,_mcpImageUrls:pictureUrls};
  }
...
async function mcpProcessMessage(req, body) {
  ...
  if (method === 'initialize') {
    return {protocol,status:200,payload:{jsonrpc:'2.0',id,result:{
      ...
      instructions:'Servidor MCP de TELOCONSIGO y TOP SHOP. Para responder preguntas, usar meli_question_context y revisar titulo, atributos, descripcion, fotos y preguntas/respuestas anteriores. Las fotos se entregan como imagenes analizables. No inventar informacion ausente. Firma de TLC: TELOCONSIGO, siempre una sola palabra. Firma de TOP SHOP: TOP SHOP. Las herramientas que modifican Mercado Libre exigen confirm=true y deben usarse solo despues de mostrar el cambio exacto y recibir confirmacion explicita del usuario.'
    }}};
  }
