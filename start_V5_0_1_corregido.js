...
const MCP_SERVER_VERSION = '5.0.1';
...
const MCP_TOOLS = [
  ...
  { name:'meli_ads_adgroups', title:'Detalle Product Ads', description:'Obtiene Ad Groups y metricas de una campana Product Ads para analizar rendimiento por producto/grupo. Solo lectura.', inputSchema:{type:'object',properties:{cuenta:{type:'string',enum:['tlc','topshop']},campaign_id:{type:['string','number']},date_from:{type:'string'},date_to:{type:'string'}},required:['cuenta','campaign_id'],additionalProperties:false}, annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:true} },
  ...
  { name:'meli_update_publication', title:'Modificar publicacion', description:'Modifica precio, stock o estado de una publicacion. Solo ejecuta si confirm=true. Para stock de publicaciones con variantes puede requerir sku.', inputSchema:{type:'object',properties:{cuenta:{type:'string',enum:['tlc','topshop']},id:{type:'string'},price:{type:'number',minimum:0},stock:{type:'integer',minimum:0},status:{type:'string',enum:['active','paused']},sku:{type:'string'},confirm:{type:'boolean',description:'Debe ser true despues de confirmacion explicita del usuario.'}},required:['cuenta','id','confirm'],additionalProperties:false}, annotations:{readOnlyHint:false,destructiveHint:true,idempotentHint:true,openWorldHint:true} }
];
...
async function mcpCallTool(name, args={}) {
  ...
  if (name === 'meli_sales') {
    ...
    // Primero usamos el filtro oficial de orders/search. Algunas cuentas/policies de ML
    // pueden rechazar la busqueda con rango de fechas aunque permitan leer las ordenes.
    // En ese caso hacemos fallback seguro: paginamos ventas recientes y filtramos localmente.
    try {
      const qs=new URLSearchParams({seller:String(sellerId),sort:'date_desc',limit:String(pageLimit),offset:String(requestedOffset)});
      if(args.date_from) qs.set('order.date_created.from', String(args.date_from));
      if(args.date_to) qs.set('order.date_created.to', String(args.date_to));
      data=await meliApi(cuenta, `/orders/search?${qs.toString()}`);
      rows=Array.isArray(data?.results)?data.results:[];
    } catch (e) {
      primaryError=e.message;
      if(!hasDateFilter) throw e;
      fallbackUsed=true;
      const collected=[];
      const fetchSize=50;
      const maxPages=20; // hasta 1000 ordenes, suficiente para el analisis operativo por periodo.
      for(let page=0; page<maxPages; page++){
        const pageOffset=page*fetchSize;
        const qs=new URLSearchParams({seller:String(sellerId),sort:'date_desc',limit:String(fetchSize),offset:String(pageOffset)});
        const chunk=await meliApi(cuenta, `/orders/search?${qs.toString()}`);
        const chunkRows=Array.isArray(chunk?.results)?chunk.results:[];
        if(!chunkRows.length) break;
        collected.push(...chunkRows);
        if(Number.isFinite(fromMs)){
          const oldest=Math.min(...chunkRows.map(o=>Date.parse(String(o?.date_created||''))).filter(Number.isFinite));
          if(Number.isFinite(oldest) && oldest < fromMs) break;
        }
        if(chunkRows.length<fetchSize) break;
      }
      const filtered=collected.filter(o=>{
        const t=Date.parse(String(o?.date_created||''));
    ...
    return {ok:true,cuenta,read_only:true,summary,fallback_used:fallbackUsed,primary_error:fallbackUsed?primaryError:undefined,...data};
  }
  ...
  if (name === 'meli_ads_adgroups') {
    ...
    const adv=await meliApi(cuenta, '/advertising/advertisers?product_id=PADS', {headers:{'Api-Version':'1','Content-Type':'application/json'}});
    const advertisers=Array.isArray(adv?.advertisers)?adv.advertisers:(Array.isArray(adv)?adv:[]); const advertiser=advertisers.find(a=>String(a.site_id||'').toUpperCase()==='MLU')||advertisers[0];
    if(!advertiser)throw new Error('La cuenta no devolvio advertiser PADS.'); const site=String(advertiser.site_id||'MLU');
    const qs=new URLSearchParams(); if(args.date_from)qs.set('date_from',String(args.date_from)); if(args.date_to)qs.set('date_to',String(args.date_to)); qs.set('metrics','clicks,prints,cost,cpc,ctr,direct_amount,indirect_amount,total_amount,direct_units_quantity,indirect_units_quantity,units_quantity,direct_items_quantity,indirect_items_quantity,advertising_items_quantity,organic_units_quantity,organic_units_amount,organic_items_quantity,acos,sov,roas,cvr,tacos');
    const data=await meliApi(cuenta, `/advertising/${encodeURIComponent(site)}/product_ads/campaigns/${encodeURIComponent(campaignId)}/ad_groups/metrics?${qs.toString()}`, {headers:{'api-version':'2'}});
    return {ok:true,cuenta,read_only:true,campaign_id:campaignId,data};
  }
  ...
  if (name === 'meli_update_publication') {
    ...
    if(!hasPrice && !hasStock && !hasStatus) throw new Error('Indica al menos price, stock o status.');
    const before=await getMeliItemForSmartUpdate(cuenta,id);
    let updateResult=null;
    if(hasPrice || hasStock) updateResult=await smartUpdateLinkedPublication(cuenta,id,{price:hasPrice?args.price:undefined,stock:hasStock?args.stock:undefined,sku:args.sku});
    let statusResult=null;
    if(hasStatus) statusResult=await meliApi(cuenta, `/items/${encodeURIComponent(id)}`, {method:'PUT',body:{status:String(args.status)}});
    const after=await getMeliItemForSmartUpdate(cuenta,id);
    return {ok:true,cuenta,action:'update_publication',item_id:id,before:{price:before?.price,available_quantity:before?.available_quantity,status:before?.status},requested:{price:hasPrice?args.price:undefined,stock:hasStock?args.stock:undefined,status:hasStatus?String(args.status):undefined,sku:args.sku||undefined},updateResult,statusResult,after:{price:after?.price,available_quantity:after?.available_quantity,status:after?.status}};
  }
